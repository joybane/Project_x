...
func fetchBSEPriceVolume() (int, error) {
	latest, err := latestDateInTable("D:/project_x/bse/price_volume", "trade_date")
	if err != nil {
		return 0, err
	}
	today := time.Now()
	...
	return mergeRowsIntoTable("D:/project_x/bse/price_volume", columns, allRows, 0, []string{"trade_date", "symbol"})
}
