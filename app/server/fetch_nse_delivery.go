...
func fetchNSEDelivery() (int, error) {
	latest, err := latestDateInTable("D:/project_x/nse/delivery", "trade_date")
	if err != nil {
		return 0, err
	}
	today := time.Now()
	...
	return mergeRowsIntoTable("D:/project_x/nse/delivery", columns, allRows, 0, []string{"trade_date", "symbol", "series"})
}
