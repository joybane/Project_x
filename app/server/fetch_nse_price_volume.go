...
func fetchNSEPriceVolume() (int, error) {
	latest, err := latestDateInTable("c:/Workdesk/project_x/nse/price_volume", "trade_date")
	if err != nil {
		return 0, err
	}
	today := time.Now()
	...
	return mergeRowsIntoTable("c:/Workdesk/project_x/nse/price_volume", columns, allRows, 0, []string{"trade_date", "symbol", "series"})
}
