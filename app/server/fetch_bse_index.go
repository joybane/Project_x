...
func fetchBSEIndexSensex() (int, error) {
	latest, err := latestDateInTable("c:/Workdesk/project_x/bse/index_sensex", "trade_date")
	if err != nil {
		return 0, err
	}
	today := time.Now()
	...
	return mergeRowsIntoTable("c:/Workdesk/project_x/bse/index_sensex", columns, rows, 0, []string{"trade_date"})
}
